...
COLS = {"ID": 4, "Name": 22, "Amount": 12, "Category": 14, "Date": 12, "Note": 20}
...
def print_table(expenses: list[Expense]) -> None:
    ...
    for e in expenses:
        print(_row(str(e.id), e.name, e.amount_str(), e.category, str(e.date), e.note or "—"))
    print(f"{CYAN}{_sep()}{RESET}")
    total = sum(e.amount for e in expenses)
    print(f"{BOLD}{GREEN}  {len(expenses)} expense(s)  |  Total: Rs.{total:,.2f}{RESET}\n")
...
def print_filter_header(
    ...
    parts = []
    if category: parts.append(f"category={YELLOW}{category}{RESET}")
    if start:    parts.append(f"from={YELLOW}{start}{RESET}")
    if end:      parts.append(f"until={YELLOW}{end}{RESET}")
